...
def model(use_data=False):
    ...
    if use_data:
        time, u, y_data, eo_data, eo_coll = get_data('trial1.csv')
    ...
    def objective(params):
        ...
        K = 0.02077936 #0.01000502 # 0.01 params[0]
        tau = 0.30082881 #2.08396  # 0.08 params[1]
        A = 0.00523523 # 0.0134 params[2]
        B = -4.14525173 #-6      # -0.07  #params[3]
        ...
        for i, t in enumerate(time):
            if i > 0:
                ...
                u_prev = u[i]
                eo_prev = eo_model[i]
                #print(i,eo_frac[i],eo_data[i])
                #print(i,y_model[i],y_data[i])
                sse = sse +  100*(eo_frac[i] - eo_data[i])**2
                sse = sse + 100*(y_model[i] - y_data[i])**2
                sse = sse + 100000*(eo_model[i]-eo_coll[i])**2
        return sse
    params0 = [0.02077936,0.30082881,0.00523523,-4.14525173]
    #params0 = -1
    #print(objective(params0))
    #res = minimize(objective, params0)
    #print(res)

    res = minimize(objective, params0, bounds=[(0,5),(0,30),(-5,5),(-10,5)])
    #print(res)
    K,tau,A,B = res['x']
